addEventListener("DOMContentLoaded", function(e){
  ...
  .then(data => {
    this.document.getElementById("divmetodos").innerHTML = data;
  })
})

document.getElementById("formMetodosEstudo").addEventListener("submit", function(e) {
  e.preventDefault(); // Evita o envio padrão do formulário

  // Obtendo os valores dos campos
  const nomeMetodo = document.getElementById("nomeMetodo").value;
  const descricaoMetodo = document.getElementById("descricaoMetodo").value;

  // Verificando se os campos estão vazios
  if (!nomeMetodo || !descricaoMetodo) {
      alert("Por favor, preencha todos os campos!");
      return;
  }

  // Exemplo de envio via AJAX usando fetch
  const data = {
  ...
  // Envio do formulário via POST (alterar URL para onde deseja enviar os dados)
  fetch('/metodo/insert', {
      method: 'POST',
      headers: {
  ...
  .then(data => {
      ...
      alert("Método de estudo salvo com sucesso!");
      
      // Limpar o formulário após o envio
      document.getElementById("formMetodosEstudo").reset();
  })
  .catch((error) => {
      // Caso ocorra um erro
      console.error('Erro:', error);
      alert("Erro ao salvar o método de estudo.");
  });
});
